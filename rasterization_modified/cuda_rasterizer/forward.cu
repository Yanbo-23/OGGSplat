...
renderCUDA(
	...
	int* __restrict__ contrib_indices,         // [H * W * MAX_CONTRIBS_PER_PIXEL]
	uint32_t* __restrict__ contrib_counts      // [H * W]
	)
{
	...
	float weight = 0;
	float D = 0;
	uint32_t contrib_count = 0;  // 👈 用于记录有效贡献个数
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// 记录有效贡献的 point_id
			if (inside && contrib_count < 64)
			{
				// contrib_indices[pix_id * 256 + contrib_count] = collected_id[j];
				contrib_indices[contrib_count * H * W + pix_id] = collected_id[j];
				contrib_count++;
			}
	...
	if (inside)
	{
		...
		// 写出该像素实际贡献的个数
		contrib_counts[pix_id] = contrib_count;
	}
