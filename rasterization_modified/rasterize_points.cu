	...
	const bool include_feature)
{
  ...
  auto uint_opts = means3D.options().dtype(torch::kUInt32);
  ...
  torch::Tensor contrib_indices = torch::full({64, H, W}, -1.0, int_opts);
  torch::Tensor contrib_counts = torch::full({1, H, W}, 0.0, uint_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		contrib_indices.contiguous().data<int>(),
		contrib_counts.contiguous().data<uint32_t>(),
		radii.contiguous().data<int>(),
		debug,
		include_feature);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, contrib_indices, contrib_counts, out_language_feature, radii, geomBuffer, binningBuffer, imgBuffer);
}
