...
def build_quaternion(R):
    """
    Convert batch of rotation matrices [B, 3, 3] to quaternions [B, 4] in the format [r, x, y, z].
    """
    B = R.shape[0]
    ...
    mask = (R[:, 0, 0] > R[:, 1, 1]) & (R[:, 0, 0] > R[:, 2, 2]) & ~mask
    t = torch.sqrt(1.0 + R[mask, 0, 0] - R[mask, 1, 1] - R[mask, 2, 2]) * 2
    q[mask, 0] = (R[mask, 2, 1] - R[mask, 1, 2]) / t
    q[mask, 1] = 0.25 * t
    q[mask, 2] = (R[mask, 0, 1] + R[mask, 1, 0]) / t
    q[mask, 3] = (R[mask, 0, 2] + R[mask, 2, 0]) / t

    # Case 3: R[1,1] is the largest diagonal term
    mask2 = (R[:, 1, 1] > R[:, 2, 2]) & ~mask & ~((R[:, 0, 0] > R[:, 1, 1]) & (R[:, 0, 0] > R[:, 2, 2]))
    t = torch.sqrt(1.0 + R[mask2, 1, 1] - R[mask2, 0, 0] - R[mask2, 2, 2]) * 2
    ...
    mask3 = ~mask & ~mask2 & ~((R[:, 0, 0] > R[:, 1, 1]) & (R[:, 0, 0] > R[:, 2, 2]))
    t = torch.sqrt(1.0 + R[mask3, 2, 2] - R[mask3, 0, 0] - R[mask3, 1, 1]) * 2
