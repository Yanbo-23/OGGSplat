...
def rescale_image_depthmap(image, feat, depthmap, camera_intrinsics, output_resolution, force=True):
    ...
    if scale_final >= 1 and not force:  # image is already smaller than what is asked
        return (image.to_pil(), feat, depthmap, camera_intrinsics)
    ...
    feat = F.interpolate(feat, size=(output_resolution[1], output_resolution[0]), mode='bilinear', align_corners=False).squeeze(0) 
    feat = feat.permute(1,2,0).contiguous()
    # print(rgb_image.shape,feat.shape)
...
def crop_image_depthmap(image, feat, depthmap, camera_intrinsics, crop_bbox):
    """
